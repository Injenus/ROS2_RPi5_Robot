...
class DualCameraPublisher(Node):
    def __init__(self):
        ...
        self.cam_fisheye = Rpi_Camera(id=0, resolution=(1440,720), name='fisheye',hard_resize_koeff=5, rotate=180, hard_roi=(612,0,1332,1440), calib_data=None, gains_roi=(0.33,0.94,0.34,0.045))
        
        self.timer_standart = self.create_timer(0.1, self.publish_image_camera_standart)  # Публикация 10 FPS
        self.timer_fisheye = self.create_timer(0.1, self.publish_image_camera_fisheye)
    ...
    def publish_image_camera_fisheye(self):
        ...
        if self.cam_fisheye.frame is not None:
            ...
            image_message = self.bridge.cv2_to_imgmsg(self.cam_fisheye.frame, encoding="bgr8")
